...
export const Settings: React.FC<Props> = ({ settings, onUpdate, onClose }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute inset-0" style={{ background: 'rgba(26,26,46,0.8)', backdropFilter: 'blur(4px)' }}/>
      <div
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold" style={{ color: '#f5f0e8' }}>settings</h2>
          <button onClick={onClose} style={{ color: '#4a4e69' }} className="hover:text-white transition-colors">
              ...
              {(['dark', 'light'] as const).map(t => (
                <button
                  ...
                  style={settings.theme === t
                    ? { background: '#f7a8b8', color: '#1a1a2e' }
                    : { background: 'rgba(247,168,184,0.1)', color: '#4a4e69' }
                  }
                >
                  {t}
                </button>
          ...
          {[
            ...
            <SettingRow key={key} label={label}>
              <button
                ...
                style={{ background: settings[key] ? '#f7a8b8' : 'rgba(74,78,105,0.4)' }}
              >
                <div
